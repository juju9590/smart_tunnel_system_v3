# ==========================================
# 파일명: traffic_accident_V5_4_2.py
# 설명:
# V5.4-2 사고 판단 로직
# - IoU 완전 제거 유지
# - 사고 핵심 특징: rear_core
...
# - 같은 pair의 candidate 반복성을 추가 반영
#   * 최근 10프레임 내 candidate 3회 이상
#   * 2프레임 연속 candidate
# ==========================================
...
class AccidentDetector:
    def __init__(self):
        ...
        # 반복성 파라미터
        self.CANDIDATE_REPEAT_WINDOW = 10     # 최근 10프레임
        self.CANDIDATE_REPEAT_COUNT = 3       # 3회 이상
        self.CANDIDATE_CONSEC_GAP = 1         # 2프레임 연속(candidate frame 차이 1)
    ...
    def _cleanup_stale_pairs(self, frame_id):
        ...
        for key in stale_keys:
            ...
            # 점수/거리 메모리는 바로 지우지 않고 유지 가능
            # 필요시 아래도 같이 제거 가능
            # self.accident_counter.pop(key, None)
            # self.pair_memory.pop(key, None)
            # self.impact_persist_counter.pop(key, None)
    ...
    def _update_candidate_repeat(self, key, frame_id, accident_candidate):
        ...
        반환:
            ...
            pair_consecutive_candidate: 직전 frame에서도 candidate였는지
