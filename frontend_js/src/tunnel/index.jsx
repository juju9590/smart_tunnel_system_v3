...
function getEventClass(event) {
  const action = typeof event === "object" ? event?.action : "";
  const text = getEventText(event);

  if (action === "accident_confirmed" || text.includes("[사고 확정]")) {
    return "event-confirmed";
  }
  if (action === "false_alarm" || text.includes("[이상 없음]")) {
    return "event-normal";
  }
  if (text.includes("사고 감지") || text.includes("[사고 감지]")) {
    return "event-detected";
  }
  return "";
}

function TunnelDashboard({ host }) {
  const [status, setStatus] = useState({
    ...
    source_name: "-",
    demo_video_key: null,
    demo_video_filename: null,
    demo_video_paused: false,
    latest_event: null,
    event_resolved: false,
    smoke_fire_candidate: false,
    smoke_fire_area_ratio: 0,
    smoke_fire_max_area: 0,
    smoke_fire_bbox: [],
    smoke_fire_map: {},
    reasons: "",
    dwell_times: {},
  ...
  const [demoLoadingKey, setDemoLoadingKey] = useState("");
  ...
  const applyStatusData = (data) => {
    ...
    setStatus({
      ...
      accident_locked: Boolean(data?.accident_locked ?? false),
      ...
      event_log_entries: Array.isArray(data?.event_log_entries) ? data.event_log_entries : [],
      frame_id: Number(data?.frame_id ?? 0),
      cctv_name: data?.cctv_name ?? "-",
      cctv_url: data?.cctv_url ?? "",
      source_type: data?.source_type ?? "cctv",
      source_name: data?.source_name ?? data?.cctv_name ?? "-",
      demo_video_key: data?.demo_video_key ?? null,
      demo_video_filename: data?.demo_video_filename ?? null,
      demo_video_paused: Boolean(data?.demo_video_paused ?? false),
      latest_event: data?.latest_event ?? null,
      event_resolved: Boolean(data?.event_resolved ?? false),
      smoke_fire_candidate: Boolean(data?.smoke_fire_candidate ?? false),
      smoke_fire_area_ratio: Number(data?.smoke_fire_area_ratio ?? 0),
      smoke_fire_max_area: Number(data?.smoke_fire_max_area ?? 0),
      smoke_fire_bbox: Array.isArray(data?.smoke_fire_bbox) ? data.smoke_fire_bbox : [],
      smoke_fire_map: data?.smoke_fire_map ?? {},
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        const demoRes = await getDemoVideos(BACKEND_URL);
        if (mounted && demoRes?.ok && Array.isArray(demoRes?.videos)) {
          setDemoVideos(demoRes.videos);
        }
  ...
  useEffect(() => {
    const event = status.pending_accident_event || status.latest_event;
    if (!event?.event_id) return;
    if (status.event_resolved) return;
    if (resolvedAccidentIds.has(event.event_id)) return;
    if (accidentModal?.event_id === event.event_id) return;

    setAccidentModal(event);
  }, [
    status.pending_accident_event,
    status.latest_event,
  ...
  const handleDemoSelect = async (video) => {
    if (!video?.key && !video?.filename) return;

    try {
      setDemoLoadingKey(video.key || video.filename);
      setError("");
      setDemoMessage("");

      stopVideo();
      setAccidentModal(null);
      setResolvedAccidentIds(new Set());
      await selectDemoVideo(BACKEND_URL, video.key || video.filename);

      const refreshed = await fetchTunnelStatus(BACKEND_URL);
      applyStatusData(refreshed);
      setDemoMessage(`${video.name} 선택됨`);
      restartVideo(200, true);
    } catch (err) {
      console.error("demo video select error:", err);
      setVideoLoading(false);
      setError("시연 영상 선택 실패");
    } finally {
      setDemoLoadingKey("");
    }
  };
  ...
  const handleDemoPauseToggle = async () => {
    if (status.source_type !== "demo_video") return;

    try {
      setLoading(true);
      setError("");

      if (status.demo_video_paused) {
        await resumeDemoVideo(BACKEND_URL);
      } else {
        await pauseDemoVideo(BACKEND_URL);
      }

      const refreshed = await fetchTunnelStatus(BACKEND_URL);
      applyStatusData(refreshed);
    } catch (err) {
      console.error("demo pause toggle error:", err);
      setError("시연 영상 일시정지/재생 실패");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResolveAccident = async (action) => {
    ...
    try {
      ...
      const refreshed = await fetchTunnelStatus(BACKEND_URL);
      applyStatusData(refreshed);
      if (data?.response_log) {
        const responseText = getEventText(data.response_log);
        setStatus((prev) => {
          const hasEntry = (prev.event_log_entries || []).some(
            (event) => getEventText(event) === responseText
          );
          const hasLog = (prev.event_logs || []).some((event) => event === responseText);
          return {
            ...prev,
            event_log_entries: hasEntry
              ? prev.event_log_entries
              : [...(prev.event_log_entries || []), data.response_log].slice(-10),
            event_logs: hasLog
              ? prev.event_logs
              : [...(prev.event_logs || []), responseText].slice(-10),
          };
        });
      }
  ...
  const eventDisplayItems = useMemo(() => {
    if (status.event_log_entries && status.event_log_entries.length > 0) {
      return status.event_log_entries;
    }
    if (status.event_logs && status.event_logs.length > 0) {
      return status.event_logs;
    }
    return status.events || [];
  }, [status.event_log_entries, status.event_logs, status.events]);
        ...
        <section className="panel panel-header">
          <div className="panel-header-left">
            ...
            <div className="panel-subtitle">
              {status.source_name || status.cctv_name || lastSelectedCctv?.name || "-"}
            </div>
          </div>

          <div className="panel-header-right">
            <div className="demo-video-select">
              <span className="demo-video-label">시연 영상 선택</span>
              <div className="demo-video-buttons">
                {demoVideos.map((video) => (
                  <button
                    key={video.key}
                    className={`demo-video-btn ${
                      status.demo_video_key === video.key ? "active" : ""
                    }`}
                    onClick={() => handleDemoSelect(video)}
                    disabled={Boolean(demoLoadingKey)}
                  >
                    {demoLoadingKey === video.key ? "전환중..." : video.name}
                  </button>
                ))}
              </div>
              {demoMessage && <span className="demo-video-message">{demoMessage}</span>}
            </div>
            {status.source_type === "demo_video" && (
              <button
                className="action-btn demo-pause-btn"
                onClick={handleDemoPauseToggle}
                disabled={loading || Boolean(demoLoadingKey)}
              >
                {status.demo_video_paused ? "시연재생" : "시연일시정지"}
              </button>
            )}
            <button
  ...
  if (!showDashboard) {
    return (
      <TunnelLanding
        ...
        portfolioUrl="https://juju9590.github.io/smart-tunnel-portfolio/"
        githubUrl="https://github.com/juju9590/smart_tunnel_system_v3"
      />
    );
  }

  return <TunnelDashboard host={host} />;
}
