# ==========================================
# 파일명: routes.py
# 위치: backend_flask/tunnel/routes.py
# 역할:
# - health
# - 캐시된 CCTV 목록 조회
...
# ---------------------------------------------------------
# Blueprint 생성
# app.py에서 url_prefix="/api/tunnel" 로 등록하므로
# 실제 주소 예:
#   /api/tunnel/health
#   /api/tunnel/status
...
def cctv_list():
    """
    ITS 최신 CCTV 리스트 조회 후 캐시된 CCTV 리스트 반환
    """
    refreshed = service.fetch_its_cctv_list()
    data = refreshed.get("items") if isinstance(refreshed, dict) else None
    if not data:
        data = service.get_cctv_list()

    return jsonify({
        "ok": True,
        "source": refreshed.get("source", "cache") if isinstance(refreshed, dict) else "cache",
        "message": refreshed.get("message", "") if isinstance(refreshed, dict) else "",
        "count": len(data),
        "items": data
    })
...
@tunnel_bp.route("/demo-videos", methods=["GET"])
def demo_videos():
    """
    raw_video 폴더에 실제 존재하는 시연 영상 목록 조회
    """
    try:
        videos = service.get_demo_videos()
        return jsonify({
            "ok": True,
            "videos": videos
        })
        ...
        return jsonify({
            "ok": False,
            "message": f"시연 영상 목록 조회 중 오류: {str(e)}"
        }), 500


@tunnel_bp.route("/select-demo-video", methods=["POST"])
def select_demo_video():
    """
    시연 영상 선택
    body 예:
      {"key": "gubong_accident"}
    ...
    if not key and not filename:
        return jsonify({
            "ok": False,
            "message": "key 또는 filename이 필요합니다."
        }), 400

    try:
        result = service.select_demo_video(key=key, filename=filename)
    except Exception as e:
        return jsonify({
            "ok": False,
            "message": f"시연 영상 선택 중 오류: {str(e)}"
        }), 500

    status_code = 200 if result.get("ok", False) else 404
    return jsonify(result), status_code


@tunnel_bp.route("/demo-video/pause", methods=["POST"])
def demo_video_pause():
    try:
        result = service.pause_demo_video()
    except Exception as e:
        return jsonify({
            "ok": False,
            "message": f"시연 영상 일시정지 중 오류: {str(e)}"
        }), 500
...
@tunnel_bp.route("/demo-video/resume", methods=["POST"])
def demo_video_resume():
    try:
        result = service.resume_demo_video()
    except Exception as e:
        return jsonify({
            "ok": False,
            "message": f"시연 영상 재생 중 오류: {str(e)}"
        }), 500
