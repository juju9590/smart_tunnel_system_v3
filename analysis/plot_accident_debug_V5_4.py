# ==========================================
# 파일명: plot_accident_debug_V5_4.py
# 위치: analysis/
# 설명:
# - accident_debug_v5_4 CSV를 읽어서
...
def aggregate_by_frame(df):
    ...
    rename_map = {
        ...
        "rear_sum": "rear_count",
        "abnormal_pose_sum": "abnormal_pose_count",
        "post_evidence_sum": "post_evidence_count",
        "impact_persist_strong_sum": "impact_persist_strong_count",
...
def summarize_file(frame_df, raw_df, source_name):
    summary = {
        ...
        "total_pairs": int(frame_df["pair_count"].sum()) if "pair_count" in frame_df.columns else 0,
        "total_candidates": int(frame_df["candidate_count"].sum()) if "candidate_count" in frame_df.columns else 0,
        "total_confirmed": int(frame_df["confirmed_count"].sum()) if "confirmed_count" in frame_df.columns else 0,
        "frames_with_candidate": int((frame_df["candidate_count"] > 0).sum()) if "candidate_count" in frame_df.columns else 0,
        "frames_with_confirmed": int((frame_df["confirmed_count"] > 0).sum()) if "confirmed_count" in frame_df.columns else 0,
        "max_score": float(frame_df["score_max"].max()) if "score_max" in frame_df.columns else 0.0,
        "mean_score": float(frame_df["score_mean"].mean()) if "score_mean" in frame_df.columns else 0.0,
        "max_lane_count": float(frame_df["lane_count"].max()) if "lane_count" in frame_df.columns else 0.0,
        "template_confirmed_frames": int(frame_df["template_confirmed"].sum()) if "template_confirmed" in frame_df.columns else 0,
    }
    return summary
...
def plot_per_file(frame_df, source_name, save_dir):
    ...
    plt.plot(frame_df["frame_id"], frame_df.get("pair_count", 0), label="pair_count")
    plt.plot(frame_df["frame_id"], frame_df.get("candidate_count", 0), label="candidate_count")
    plt.plot(frame_df["frame_id"], frame_df.get("confirmed_count", 0), label="confirmed_count")
    plt.xlabel("frame_id")
    plt.ylabel("count")
    ...
    # -----------------------------
    plt.figure(figsize=(14, 6))
    plt.plot(frame_df["frame_id"], frame_df.get("rear_count", 0), label="rear_count")
    plt.plot(frame_df["frame_id"], frame_df.get("abnormal_pose_count", 0), label="abnormal_pose_count")
    plt.plot(frame_df["frame_id"], frame_df.get("post_evidence_count", 0), label="post_evidence_count")
    plt.plot(frame_df["frame_id"], frame_df.get("impact_persist_strong_count", 0), label="impact_persist_strong_count")
    plt.xlabel("frame_id")
    plt.ylabel("count")
    ...
    # -----------------------------
    plt.figure(figsize=(14, 6))
    plt.plot(frame_df["frame_id"], frame_df.get("score_max", 0), label="score_max")
    plt.plot(frame_df["frame_id"], frame_df.get("score_mean", 0), label="score_mean")
    plt.xlabel("frame_id")
    plt.ylabel("score")
    ...
    # -----------------------------
    plt.figure(figsize=(14, 6))
    plt.plot(frame_df["frame_id"], frame_df.get("lane_count", 0), label="lane_count")
    plt.plot(frame_df["frame_id"], frame_df.get("template_confirmed", 0), label="template_confirmed")
    plt.xlabel("frame_id")
    plt.ylabel("value")
